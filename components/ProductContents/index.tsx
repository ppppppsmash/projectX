...
export default function ProductContents() {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 flex-wrap max-w-full mx-auto">
      ...
      <div
        key={index}
        className="px-2 py-3 w-ful/l h-[220px] md:w-full md:h-[260px] md:max-w-[280px] mx-auto
      ...
      >
        ...
        <div className="max-w-full mx-auto">
          <p className="w-full truncate max-w-full text-center mx-auto mt-2 md:mt-4 text-[14px] font-semibold">{data.title}</p>
          <p className="w-full max-w-full text-left mx-auto mt-2 md:mt-4 text-[12px]">{data.description}</p>
        </div>
      </div>
      ))}
    </div>
